...
def main(
    ...
    *,
    ...
    log_dir = f"logs_/{checkpoint_path.replace("/", "_")}"
    ...
    for file in os.listdir(data_dir):
        ...
        state_summary = traj_data['state_summary'][str(low_start)]
        prefix.append({"type": "text", "text": f"State: You are in the middle of a room. {state_summary};\n"})
        ...
        for eval_idx, high_idx in enumerate(range(sub_traj['high_pddl_idx'][0], sub_traj['high_pddl_idx'][1])):
            ...
            while not done:
                ...
                inputs = processor(text=prompt, images=[_image], return_tensors="pt")
...
'''
run_interactive_eval_longhrz_text_state.py --data_dir /data/infini-thor/test/traj_memory --base_model "Qwen/Qwen2.5-VL-7B-Instruct" --checkpoint /data/checkpoints/infini-memory-text-state/step-60000-hf/ --flash_attn
'''