...
def read_images(_img_dir, img_width, img_height):
    image_dict = {}
    img_dir = os.path.join(_img_dir, "img")
...
def interact_with_env(env, agent, action, eval_idx):
    ...
    if not t_success:
        logger.info(f"FAIL -- action: {action}, errorMessage: {env.last_event['errorMessage']}")
...
def main(
    ...
    *,
    ...
    img_tok_id = processor.tokenizer.convert_tokens_to_ids(image_token)
    
    # from torchtitan.datasets.infini_thor_dataset import InfiniTHORDataset
    # dataset_cls = InfiniTHORDataset
    ...
    for file in os.listdir(data_dir):
        ...
        if traj_id in existing_id:
            logger.info(f"Skipping traj_id={traj_id} (already present in {log_path})")
            continue
        else:
            metric = {
                "total_count": 0, "success_count": 0, "agent_reward": 0,
        ...
        img_tar_file = file.replace("txt", "tar") if "txt" in file else file.replace("json", "tar")
        tar_file = os.path.join(img_data_dir, img_tar_file)
        if os.path.exists(tar_file):
            img_dict = extract_and_convert_tar(tar_file, 336, 336)
        elif os.path.isdir(os.path.join(os.path.join(img_data_dir, img_tar_file.split(".")[0]))):
            img_dir = os.path.join(os.path.join(img_data_dir, img_tar_file.split(".")[0]))
            img_dict = read_images(img_dir, 336, 336)
        ...
        #for sub_task, sub_traj in zip(traj_data['sub_tasks'][:1], traj_data['sub_trajs'][:1]):
            ...
            for eval_idx, high_idx in enumerate(range(sub_traj['high_pddl_idx'][0], sub_traj['high_pddl_idx'][1])):
                ...
                logger.info(f" ==== replay: high_idx: {high_idx}, {traj_data['plan']['high_pddl'][high_idx]}")
        ...
        sub_task = traj_data['sub_tasks'][-1]
        sub_traj = traj_data['sub_trajs'][-1]
        ...
        metric["total_count"] += num_subgoals
        for ep in expert_plan:
            if ep['discrete_action']['action'] == "GotoLocation":
                metric['goto_count'] += 1
            elif ep['discrete_action']['action'] == "PickupObject":
                metric['pick_count'] += 1
            elif ep['discrete_action']['action'] == "PutObject":
                metric['put_count'] += 1
        ...
        for low_idx, entry in enumerate(memory_imgs):
            ...
            if entry in img_dict:
                prefix.append({"type": "image", "image": img_dict[entry]})
        ...
        for eval_idx, high_idx in enumerate(range(sub_traj['high_pddl_idx'][0], sub_traj['high_pddl_idx'][1])):
            ...
            step = 0            
            while not done:
                contents = copy.deepcopy(prefix)

                buffer = io.BytesIO(base64.b64decode(env.last_event['frame_bytes']))
                buffer.seek(0)
                _image = Image.open(buffer)

                contents.append({"type": "text", "text": f"Current state: "})
                contents.append({"type": "image", "image": _image})
                ...
                inputs = processor(text=prompt, images=imgs + [_image], return_tensors="pt")
                ...
                if success and done:
                    ...
                    if expert_plan[eval_idx]['discrete_action']['action'] == "GotoLocation":
                        metric['goto_success'] += 1
                    elif expert_plan[eval_idx]['discrete_action']['action'] == "PickupObject":
                        metric['pick_success'] += 1
                    elif expert_plan[eval_idx]['discrete_action']['action'] == "PutObject":
                        metric['put_success'] += 1
