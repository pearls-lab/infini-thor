...
def build_messages(history, lang_input, img_list):

    content = []

    if len(history) <= 1:
        content.append({"type": "text", "text": f"Action history: No history is available.\n"})
    else:
        history_str = "; ".join(history[:-1])
        content.append({"type": "text", "text": f"Action history: {history_str};\n"})

    content.append({"type": "text", "text": f"Current state: "})
            ...
            for eval_idx, high_idx in enumerate(range(sub_traj['high_pddl_idx'][0], sub_traj['high_pddl_idx'][1])):
                ...
                if last_step <= expert.step + len(cur_expert_actions):
                    ...
                    while not done or agent.step < 2000:
                        messages, img_list = build_messages(agent.history, agent.traj_str, agent.img_list)
