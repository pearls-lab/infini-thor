...
def replay_json(env, traj_data):
    ...
    #             position={'x': traj_data['scene']['init_action']['x'], 'y': traj_data['scene']['init_action']['y'], 'z': traj_data['scene']['init_action']['z'],},
    #             rotation={'x': 0, 'y': traj_data['scene']['init_action']['rotation'], 'z': 0},
    #             standing=True
    #         )
    # env.step(init_act)
    #print("Task: %s" % (traj_data['template']['task_desc']))
    #print("Task: %s" % (json_file))
    ...
    return steps_taken, event

def replay_agent(agent, traj_data):
    ...
    scene_name = 'FloorPlan%d' % scene_num
    agent.env.reset(scene_name)
    agent.env.restore_scene(object_poses, object_toggles, dirty_and_empty)

    # initialize
    event = agent.env.step(dict(traj_data['scene']['init_action']))
    ...
    for ll_action in traj_data['plan']['low_actions']:
        ...
        compressed_mask = action_args['mask'] if 'mask' in action_args else None
        if compressed_mask is not None:
            mask = agent.env.decompress_mask(compressed_mask)
        else:
            mask = None
        ...
        success, event, target_instance_id, err, _ = agent.env.va_interact(action_name, interact_mask=mask)
        if not success:
            raise RuntimeError(err)
