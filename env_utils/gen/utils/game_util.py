...
def get_object_by_action_type(action_name, object_id, metadata):
    '''
    when concatenating trajs objectIds are different across the trajs.
    Need to get id by object type (a.k.a. object name)
    '''
    ...
    obj = get_object(object_id, metadata)
    if obj:
        return obj

    # if objectId is changed, (bc of position?)
    object_type = object_id.split("|")[0] 

    obj_candis = get_objects_of_type(object_type, metadata)

    if len(obj_candis) == 1:
        return obj_candis[0]
    
    condition = condition_by_action[action_name]
    for oc in obj_candis:
        if oc[condition] and oc['visible']:
            return oc
