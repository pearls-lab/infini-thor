...
import gen.utils.bb_util as bb_util
...
class TaskGameStateFullKnowledge(TaskGameState):
    ...
    def step(self, action_or_ind):
        ...
        if not self.env.last_event.metadata['lastActionSuccess']: # manually fix planners issue
            print('step action', game_util.get_action_str(action), "error: ", self.env.last_event.metadata['errorMessage'])
            if action['action'] == 'PutObject' and 'Target openable Receptacle is CLOSED' in self.env.last_event.metadata['errorMessage']:
                put_action = constants.data_dict['plan']['low_actions'].pop(-1)
                recep_obj = put_action['api_action']['receptacleObjectId']

                self.env.step({'action': 'OpenObject', 'objectId': recep_obj})
                if self.env.last_event.metadata['lastActionSuccess']:
                    self.env.step(put_action['api_action'])
                    if self.env.last_event.metadata['lastActionSuccess']:               
                        self.env.step({'action': 'CloseObject', 'objectId': recep_obj})
                        if self.env.last_event.metadata['lastActionSuccess']:
                            ...
                            print('step action', game_util.get_action_str(action), " ==> FIXED!!")
            elif action['action'] == 'PickupObject' and 'not visible' in self.env.last_event.metadata['errorMessage']:
                visible_objs = game_util.get_visible_objs(self.env.last_event.metadata['objects'])
                new_obj = None
                for vo in visible_objs:
                    if action['objectId'] in vo['objectId']: # e.g., 'Apple|-02.02|+00.96|-00.51' in 'Apple|-02.02|+00.96|-00.51|AppleSliced_2'
                        new_obj = vo['objectId']
                        break
                if new_obj:
                    new_action = constants.data_dict['plan']['low_actions'].pop(-1)
                    new_action['objectId'] = new_obj # update object ID
                    self.env.step(new_action)
                    if self.env.last_event.metadata['lastActionSuccess']:
                        constants.data_dict['plan']['low_actions'].append(new_action)
                        print('step action', game_util.get_action_str(action), " ==> fixed!!")
                else:
                    print('step action', game_util.get_action_str(action), " ==> object is not visible")
        else:
