...
class GameStateBase(object):
    ...
    def reset(self, scene_num=None, use_gt=True, seed=None, max_num_repeats=constants.MAX_NUM_OF_OBJ_INSTANCES,
        ...
        # action = {'action': 'TeleportFull',
        #         'position': {'x': self.start_point[0] * constants.AGENT_STEP_SIZE,
        #                     'y': self.agent_height,
        #                     'z': self.start_point[1] * constants.AGENT_STEP_SIZE},
    ...
    def store_ll_action(self, action, discrete_action=True):
        _update_dict = {"high_idx": len(constants.data_dict['plan']['high_pddl']) - 1,
                        "api_action": action}
        if discrete_action:
            _update_dict.update({"discrete_action": self.get_ll_discrete_action(action)})

        constants.data_dict['plan']['low_actions'].append(_update_dict)
            ...
            if 'action' not in action or action['action'] is None or action['action'] == 'None':
                ...
                if constants.RECORD_VIDEO_IMAGES:
                    ...
                    if 'Teleport' in action['action']:
                        ...
                        pickup_obj = game_util.get_object_by_action_type(action['action'], action['objectId'], self.env.last_event.metadata)
                    ...
                    self.store_ll_action(action, discrete_action=False)
                    # self.save_image(1)
                    self.event = self.env.step(action)
