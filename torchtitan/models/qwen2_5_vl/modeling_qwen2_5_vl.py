...
class Qwen2_5_VLForActionPrediction(Qwen2_5_VLForConditionalGeneration):
    ...
    def init_buffers(self, buffer_device, buffers_dict=None):
        ...
        # # TODO: check buffers if all buffers are correctly set.

        n_buffer_key = 0
        with torch.device(buffer_device):
            head_dim = self.visual.config.hidden_size // self.visual.config.num_heads
            dim, theta = head_dim // 2, 10000.0
            inv_freq = 1.0 / (theta ** (torch.arange(0, dim, 2, dtype=torch.float) / dim))
            self.visual.rotary_pos_emb.inv_freq = inv_freq
            n_buffer_key += 1
            # this model.rotary_emb is for position embeddings
            inv_freq, _ = self.model.rotary_emb.rope_init_fn(self.model.rotary_emb.config, buffer_device)
            ...
            for layer in self.model.layers: # nn.ModuleList
...
__all__ = ["Qwen2_5_VLForActionPrediction", "Qwen2_5_VLForConditionalGeneration", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel"]
