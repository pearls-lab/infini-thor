...
def apply_tp( 
    ...
    """Apply tensor parallelism (only shard along hidden dim, no SequenceParallel)."""

    cp_enabled = parallel_dims.cp_enabled  # still used for loss_parallel behavior if you like
    ...
    # ------------------------------------------------------------------
    # 0) DO NOT TP / SP the top-level norm or layernorms
    # ------------------------------------------------------------------
    # We also don't SequenceParallel model.model.norm anymore.
    # If you want, you can leave this whole call removed/commented out.
    # parallelize_module(
    #     model.model,
    #     tp_mesh,
    ...
    # ------------------------------------------------------------------
    # 1) lm_head – only depends on hidden dim, keep inputs replicated
    # ------------------------------------------------------------------
    lm_head_input_layout = Replicate()
    ...
    # ------------------------------------------------------------------
    # 2) Per-layer tensor parallel: ONLY shard hidden dim
    # ------------------------------------------------------------------
    layers = model.model.layers
    if isinstance(layers, nn.ModuleDict):
        layer_iterator = layers.items()
    else:
        layer_iterator = enumerate(layers)

    for layer_id, transformer_block in layer_iterator:
        # Unified plan: no SequenceParallel, no Shard(1).
        # Hidden states in/out of the block are Replicate().
        layer_plan = {
            # --- Attention ---
            # input_layernorm stays fully replicated by default
            # self_attn input stays Replicate, no PrepareModuleInput needed
            "self_attn.q_proj": colwise_parallel(),  # output: Shard(-1)
            "self_attn.k_proj": colwise_parallel(),
            "self_attn.v_proj": colwise_parallel(),
            # o_proj: take sharded hidden, return replicated hidden
            ...
            # --- MLP ---
            # mlp input is replicated, so we don't need PrepareModuleInput
            "mlp.gate_proj": colwise_parallel(),     # output: Shard(-1)
            "mlp.up_proj": colwise_parallel(),       # output: Shard(-1)
            "mlp.down_proj": rowwise_parallel(
                input_layouts=Shard(-1),
                output_layouts=Replicate(),
            ),

            # post_attention_layernorm also remains replicated
        }
    ...
    logger.info(
        ...
        "Tensor Parallelism to the model (hidden-dim only; no SequenceParallel)."
    )

# with freezed visual module
def apply_tp_backup(
    model: nn.Module,
    tp_mesh: DeviceMesh,
    parallel_dims: "ParallelDims",
