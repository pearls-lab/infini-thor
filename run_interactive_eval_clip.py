...
def retrieve_top_images_clip(
    ...
    Args:
        question: task goal description (text).
        img_paths: list of image file paths (historical states).
        clip_model / clip_processor / device: CLIP components.
        top_k: number of images to retrieve.

    Returns:
        List of selected image paths in chronological order.
    """
    if len(img_paths) == 0:
        return []
    ...
    for start in range(0, len(img_paths), batch_size):
        ...
        batch_imgs = []
        batch_idx = []
        for i, img in enumerate(batch_paths):
    ...
    if not all_img_feats:
        # If all images failed to load, just return original list (truncated)
        return img_paths[:top_k]

    img_features = torch.cat(all_img_feats, dim=0)
    sims = (img_features @ text_features.T).squeeze(1)

    k = min(top_k, img_features.size(0))
    _, top_indices = torch.topk(sims, k=k, largest=True, sorted=True)
    top_indices = top_indices.tolist()

    # Map back to original indices in img_paths
    top_original_indices = [valid_indices[i] for i in top_indices]
    
    # Keep chronological order for context
    chronological_indices = sorted(top_original_indices)
    chronological_imgs = [img_paths[x] for x in chronological_indices]
    return chronological_imgs
...
class TrajManager:
    ...
    def append_history(self, _history):
    ...
    def copy_from_expert(self, expert):
        ...
        self.history = expert.history.copy()
    ...
    def is_stuck(self):
        # if the agent repeats same actions esp RotateRight and RotateLeft, return True
        if len(set(self.log['action'][-10:])) == 1 and set(self.log['action'][-10:]) != set(["MoveAhead"]):
            return True
        return False
...
def build_messages(goal_str, lang_input, img_list, clip_model, clip_processor, device):

    imgs = []
    memory_imgs = []

    if len(img_list) < 20:
        memory_imgs = img_list
    else:
        memory_imgs = retrieve_top_images_clip(
            goal_str,
            img_list,
            clip_model,
            clip_processor,
    ...
    if memory_imgs:
        content.append({"type": "text", "text": f"Previous states: "})
        for low_idx, entry in enumerate(memory_imgs):
            content.append({"type": "image", "image": entry})
            imgs.append(entry)
    
    content.append({"type": "text", "text": f"\nCurrent state: "})
    content.append({"type": "image", "image": img_list[-1]})
    content.append({"type": "text", "text": f" Next action: "})
    ...
    return messages, memory_imgs + [img_list[-1]]
...
def main(
    ...
    *,
    ...
    if 'llava' in base_model.lower():
        ...
        model_cls = Qwen2_5_VLForActionPrediction
    ...
    clip_model_name = "openai/clip-vit-base-patch32"  # change if you want a different CLIP
    ...
    for file in os.listdir(data_dir):
        ...
        for sub_task, sub_traj in zip(traj_data['sub_tasks'], traj_data['sub_trajs']):
            ...
            task_goal_str = sub_task['task_desc']
            ...
            for eval_idx, high_idx in enumerate(range(sub_traj['high_pddl_idx'][0], sub_traj['high_pddl_idx'][1])):
                ...
                expert.append_history(get_templated_high_pddl_desc(traj_data['plan']['high_pddl'][high_idx]))
                ...
                if last_step <= expert.step + len(cur_expert_actions):
                    ...
                    while not done or agent.step < 2000:
                        messages, img_list = build_messages(task_goal_str, 
                            agent.traj_str,
                            agent.img_list,
                            clip_model,
                            clip_processor,
                        ...
                        if agent.is_stuck():
                            success = False
                            logger.info(f"!! agent.step: {agent.step}, agent is stuck -- break")
                            break
